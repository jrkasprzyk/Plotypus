...
def main(input=None):
    ...
    def load_set(file):
        ...
        if file is None:
            return plotypus.load(sys.stdin)

        try:
            return plotypus.load_json(file)
        except json.decoder.JSONDecodeError:
            return plotypus.load_objectives(file)

    def save_set(result, file, indent=4):
        """Output result to stdout or file."""
        if file is None:
            plotypus.dump(result, sys.stdout, indent=indent)
            sys.stdout.write(os.linesep)
        else:
            plotypus.save_json(file, result, indent=indent)
    ...
    parser = ArgumentParser(prog="plotypus",
                            description="Plotypus - Multiobjective optimization in Python")

    parser.add_argument("-v", "--version", action="version", version=plotypus.PlatypusConfig.version)
    ...
    if args.command == "hypervolume":
        ...
        hyp = plotypus.Hypervolume(reference_set=ref_set)
        print(hyp.calculate(input_set))
    elif args.command == "gd":
        ref_set = load_set(args.reference_set)
        input_set = load_set(args.filename)
        gd = plotypus.GenerationalDistance(reference_set=ref_set)
        print(gd.calculate(input_set))
    elif args.command == "igd":
        ref_set = load_set(args.reference_set)
        input_set = load_set(args.filename)
        igd = plotypus.InvertedGenerationalDistance(reference_set=ref_set)
        print(igd.calculate(input_set))
    elif args.command == "epsilon":
        ref_set = load_set(args.reference_set)
        input_set = load_set(args.filename)
        eps = plotypus.EpsilonIndicator(reference_set=ref_set)
        print(eps.calculate(input_set))
    elif args.command == "spacing":
        input_set = load_set(args.filename)
        spacing = plotypus.Spacing()
        print(spacing.calculate(input_set))
    elif args.command == "solve":
        if args.seed is not None:
            random.seed(args.seed)

        problem_module = importlib.import_module(coalesce(args.problem_module, "plotypus"))
        algorithm_module = importlib.import_module(coalesce(args.algorithm_module, "plotypus"))

        if args.problem not in dir(problem_module):
            raise plotypus.PlatypusError(f"'{args.problem}' not found in module '{problem_module.__name__}'")
        if args.algorithm not in dir(algorithm_module):
            raise plotypus.PlatypusError(f"'{args.algorithm}' not found in module '{algorithm_module.__name__}'")

        problem_class = getattr(problem_module, args.problem)
        algorithm_class = getattr(algorithm_module, args.algorithm)

        if not issubclass(problem_class, plotypus.Problem):
            raise plotypus.PlatypusError(f"'{args.problem}' is not a valid Problem")
        if not issubclass(algorithm_class, plotypus.Algorithm):
            raise plotypus.PlatypusError(f"'{args.algorithm}' is not a valid Algorithm")
        ...
        if args.unique:
            input_set = plotypus.unique(input_set)
        if args.feasible:
            input_set = plotypus.feasible(input_set)
        if args.nondominated or args.epsilons:
            archive = plotypus.EpsilonBoxArchive(args.epsilons) if args.epsilons else plotypus.Archive()
            archive += input_set
            input_set = archive
        ...
        if args.reference_set:
            ...
            ref_set = load_set(args.reference_set)
            minimum, maximum = plotypus.normalize(ref_set)

        norm_min, norm_max = plotypus.normalize(input_set, minimum, maximum)
        ...
        if nobjs == 2:
            ...
            raise plotypus.PlatypusError("plot requires a set with 2 or 3 objectives")
